...
export default function Widget(props: AllWidgetProps<{}>) {
  ...
  const handleButtonClick = () => {
    ...
    const attachmentsDictionary: { [objectId: string]: number } = {}; // Initialize dictionary
    ...
    const promises = records.map((record) => {
      ...
      return new Promise<void>((resolve, reject) => {
        const layer_url = ds.itemInfo.url; // Use ds.itemInfo.url
        ...
        layer.queryAttachments({
          ...
          attachmentsDictionary[objectIdValue] = attachmentCount; // Store attachment count in dictionary
          resolve();
        }).catch((error) => {
    ...
    Promise.all(promises).then(() => {
      setGlobalAttachmentsDictionary(attachmentsDictionary); // Update the globalAttachmentsDictionary with the new data
  ...
  // THIS IS THE PROCESSING FUNCTION TO EDIT
  // ///////////////////////////////////////
  ...
  const handleProcessData = async () => {
    ...
    for (const [objectId, attachmentCount] of Object.entries(globalAttachmentsDictionary)) {
      const checkedBoxes = [];
    ...
    const headers = ["ObjectID", "URLs", ...Object.keys(processedData[0]).filter(key => key !== "objectId" && key !== "urls")];
  ...
  if (completed) {
    return (
      <div className="maincontainer" style={{ overflowY: 'auto', maxHeight: '400px' }}>
        {Object.entries(globalAttachmentsDictionary).map(([objectId, attachmentCount]) => (
          <div key={objectId} style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', height: '10%', width: '100%', transition: '0.2s ease-in-out', ':hover': { boxShadow: '0 0 11px rgba(33, 33, 33, 0.2)' } }}>
            <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', width: '35%' }}>Point: {objectId}</div>
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'left', width: '45%', margin: '0 20%' }}>
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%'}}>
      {loading ? (
        ...
        <button onClick={handleButtonClick} style={{margin: 'auto'}}>Select Attachments</button>
      )}
    </div>
  );
};
